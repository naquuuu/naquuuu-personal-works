...
9. **WhatsApp Relay UX (ADR-027, 2026-09-25)**: the WhatsApp bots must read human — 1-3 sentences by default, no bullet menus, no reasoning/tool/meta chatter, no raw phone numbers or IDs (names only), failures in one plain sentence. Workspace questions route through `opencode run` (never answered from Hermes memory); curator-directed messages route to `opencode run --agent naquuuu-curator`. On the relay host, WhatsApp display keeps tool progress, reasoning, interim messages, and streaming **off**; persona text is versioned at `internal-docs/relay/WHATSAPP_SOUL.md` and must stay mirrored to the host.
